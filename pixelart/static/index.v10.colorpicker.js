...
xmlhttppicker.open("GET", "/v10/colors", true);
xmlhttppicker.send();	
...
picker.addEventListener('mouseup',function(evt){
	...
	if(!rightclick){
		...
		xmlhttpclick.open("GET", "/v10/colors?clickx=" + clickx + "&clicky=" + clicky, true);
		xmlhttpclick.send();
	}	
	rightclick = false;
},false);
...
picker.addEventListener('contextmenu', function(evt) {
		...
		clickx = Math.floor((evt.clientX - pickeroffsetX)/pickerscale);
		clicky = Math.floor((evt.clientY - pickeroffsetY)/pickerscale);
		xmlhttpclick.open("GET", "/v10/colors?clickx=" + clickx + "&clicky=" + clicky + "&rclick=1", true);
		xmlhttpclick.send();
	    }
